...
已实现注册、登录、退出登录、用户文件列表、MD5 上传预检、普通文件上传与下载、逻辑删除以及分享状态管理。大文件分片上传、受控分享链接、前端和 FAISS 检索仍在后续阶段。
...
测试会启动 MySQL、Redis、FastDFS tracker/storage、C FastCGI 与 Nginx，覆盖注册、登录、Redis Token、真实文件上传、下载内容校验、上传事务入库、两个用户并发上传相同内容、跨用户 MD5 认领拒绝、真实字节验证后的物理去重、文件列表、分享、取消分享、删除和退出登录。退出后会检查 Redis key 已删除、旧 Token 被拒绝，并验证重复退出可安全重试。
